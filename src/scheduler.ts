// 알림 로직 (Asia/Seoul 기준). 서버리스 전환으로 상시 프로세스·node-cron 은 제거되고,
// 외부 스케줄러(cron-job.org / Vercel Cron)가 /api/cron/* 를 호출해 아래 함수를 실행한다.
...
export async function runDailyNotifications(today = todayKst()): Promise<number> {
  ...
  for (const ipo of ipos) {
    ...
    for (const j of jobs) {
      if (await wasNotified(ipo.id, j.kind)) continue;
      ...
      if (r.sent > 0 || r.failed > 0) await markNotified(ipo.id, j.kind);
      sent += r.sent;
    }
  }
  return sent;
...
export async function runDeadlineNotifications(today = todayKst()): Promise<number> {
  ...
  for (const ipo of ipos) {
    ...
    if (await wasNotified(ipo.id, kind)) continue;
    ...
    if (r.sent > 0 || r.failed > 0) await markNotified(ipo.id, kind);
    sent += r.sent;
  }
  return sent;
}
