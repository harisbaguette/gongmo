...
- **유통가능물량 추출**: 형식이 들쭉날쭉한 "공모후 유통가능 물량" 표에서 비율(%)을 뽑는다. **코드 규칙이 먼저** 계산하고(실측 53건 중 51건 정답·오답 0), 규칙이 감당 못 하는 서식만 OpenRouter LLM 에 맡긴다(실패 시 "미확인").
...
| `SCRAPE_DEADLINE_MS` | 240000 | 수집 시간 예산. 넘기면 남은 종목을 다음 실행에 넘기고 정상 종료 |
| `OPENROUTER_API_KEY` | — | **규칙으로 못 뽑은 낯선 서식에서만** 쓰는 예비 수단(없으면 해당 값 null) |
| `OPENROUTER_MODEL` | `amazon/nova-micro-v1` | 예비 수단 모델(실측 비교로 선정 — 아래 표 참고) |
| `OPENROUTER_TIMEOUT_MS` | 15000 | 추출 요청 1건 최대 대기(넘으면 끊고 재시도) |
| `OPENROUTER_RETRY_MS` | 2000 | 429·5xx 뒤 재시도 전 대기 |
...
### 유통가능물량은 어떻게 뽑는가 (규칙 우선, 모델은 예비 + 교차 확인)

1. **규칙(코드)**: 합계 행 끝에는 [매각제한 지분율][유통가능 지분율]이 나란히 있고 둘의 합이 100%다.
   "붙어 있으면서 합이 100%인 지분율 쌍"을 마지막 합계 표기 뒤에서 찾아 뒤쪽 값을 쓴다.
   합이 100%에 더 정확한 쌍을 우선하고, 똑같이 정확하면 앞쪽(표)을 쓴다 — 뒤에 오는 것은 대개 설명문이다.
   (`extractFloatByRule`, 외부 호출 0회 · 1ms 미만)
2. **모델(예비)**: 규칙이 못 찾는 낯선 서식일 때만 OpenRouter 에 묻는다.
   두 번 물어 같을 때만 채택하고, 갈리면 세 번째로 다수결한다.
3. **교차 확인**: 값을 **처음** 채우는 종목은 모델에게 한 번 되물어, 규칙 값과 다르면 수집 요약의
   `floatMismatch` 로 세고 로그에 남긴다(값은 규칙 채택). 낯선 서식을 조기에 발견하기 위한 안전망이다.
4. 끝내 못 정하면 **값을 비운다**. 틀린 값은 등급을 조용히 오염시키지만, 빈 값은 화면에서 바로 보인다.

**실측 (2026-07-25 · 실제 공시 53건 · 2161~2309번, 회사마다 서식이 다름)**

| | 규칙 | 모델 단독 |
|---|---|---|
| 정답 | **51건** | 교차검증 30건 중 **4건 오답**(매각제한 지분율을 유통물량으로 읽음) |
| 기권 → 모델이 이어받아 정답 | 2건 | — |
| **오답** | **0건** | 4건 |

기권한 2건은 규칙이 손대면 안 되는 서식이었다: `%` 기호가 빠진 오타 공시(제이피아이헬스케어),
매각제한·유통가능이 각각 다른 소계로 갈린 서식(서울보증보험). 둘 다 모델 폴백이 정답을 냈다.

> 왜 규칙을 앞에 두는가: 모델은 같은 표에서도 앞쪽(매각제한) 값을 집는 일이 잦았다(30건 중 4건).
> 원문만 보면 확실히 계산되는 값을 모델 판단에 맡기지 않는 것이 근본 해법이다.
> 반대로 규칙이 감당 못 할 서식에서는 **값을 만들어내지 않고 기권**시켜 모델이 이어받게 했다.
> 한계: 38.co.kr 은 오래된 종목의 상세 표를 보관하지 않아, 검증 표본은 2161번 이후 공시다.
