...
### 유통가능물량은 어떻게 뽑는가 (규칙 우선, 모델은 예비)

1. **규칙(코드)**: 합계 행 끝에는 항상 [매각제한 지분율][유통가능 지분율]이 나란히 있고 둘의 합이 100%다.
   그래서 "붙어 있으면서 합이 100%가 되는 지분율 쌍" 중 **마지막 합계 표기 직후의 쌍**을 찾아 뒤쪽 값을 쓴다.
   → 실제 공시 **23건 전부 정답**, 외부 호출 0회, 종목당 1ms 미만. (`extractFloatByRule`)
2. **모델(예비)**: 규칙이 못 찾는 낯선 서식일 때만 OpenRouter 에 묻는다.
   이때는 값이 흔들릴 수 있어 두 번 물어 같을 때만 채택하고, 갈리면 세 번째로 다수결한다.
3. 끝내 못 정하면 **값을 비운다**. 틀린 값은 등급을 조용히 오염시키지만, 빈 값은 화면에서 바로 보인다.

> 왜 규칙을 앞에 두는가: 모델은 같은 표에서도 가끔 앞쪽(매각제한) 값을 집었다(2301 종목: 33.04 대신 66.96).
